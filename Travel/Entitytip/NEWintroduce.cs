...
namespace Entitytip
{
    public class NEWintroduce
    {
        ...
        public static NEWintroduce create(string English,string name,string TravelType,string time,string language, string ticketname,string sicketmoney,string TxTmessage,string image1, string image2, string image3, string image4)
        {
            DbHelper db = new DbHelper();
            DbCommand cmd = db.GetSqlStringCommond("insert into NewIntroduce(SceneID,name,TravelType,time,language,TxTmessage,image1,image2,image3,image4)values(@SceneID,@name,@TravelType,@time,@language,@TxTmessage,@image1,@image2,@image3,@image4)");
